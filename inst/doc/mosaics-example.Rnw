...
If you have the data for matched control sample, two-sample analysis
(either with mappability and GC content or without them) is usually recommended.
The two-sample analysis with mappability and GC content is especially useful
for the ChIP-seq data with low sequencing depth.
If the ChIP-seq data is deeply sequenced, the two-sample analysis is often appropriate.
...
Sections \ref{two_sample_MGC} and \ref{two_sample_no_MGC} discuss
the work flow and command lines for the two-sample analysis with mappability and GC content
and without them, respectively. Section \ref{one_sample} discuss the case
of the one-sample analysis.

`\texttt{mosaics}' package accepts aligned read files 
(such as ones obtained from the ELAND or bowtie aligner) as input
and converts them into bin-level files for modeling and visualization purposes.
...
\section{Workflow: Two-Sample Analysis}\label{two_sample_MGC}
...
constructBins( infileLoc="/scratch/eland/", infileName="STAT1_eland_results.txt", 
    fileFormat="eland_result", outfileLoc=infileLoc, 
    byChr=FALSE, fragLen=200, binSize=fragLen, capping=0 )
@

You can specify the directory, name, and file format of the aligned read file 
in `\texttt{infileLoc}', `\texttt{infileName}', and `\texttt{fileFormat}' arguments, respectively.
...
to a separate file named as `\texttt{[chrID]}$\_$\texttt{[infileName]}$\_$\texttt{fragL[fragLen]}$\_$\texttt{bin[binSize].txt}',
...
By default, constructed bin-level files are exported to the directory 
that the aligned read file is located at. 
If you prefer to export them to another directory, you can specify it in `\texttt{outfileLoc}' argument.

\subsection{Reading  Bin-Level Data into the R Environment}\label{readBins}

For the two-sample analysis with mappability and GC content,
you need preprocessed bin-level ChIP data, control sample data, 
...
and it is recommended for most cases.
The chromosome-wise analysis is usually faster than the genome-wide analysis.
...
exampleFit <- mosaicsFit( exampleBinData, analysisType="TS", bgEst=NA )
@
...
`\texttt{analysisType="TS"}' indicates implementation of the two-sample analysis.
...
If `\texttt{bgEst=NA}' (default), `\texttt{mosaicsFit}' tries its best guess 
for the background estimation approach, based on the data provided.
If the goodness of fit obtained using `\texttt{bgEst=NA}' is not satisfactory,
we recommend to try `\texttt{bgEst="matchLow"}' and/or `\texttt{bgEst="rMOM"}' 
...
\section{Two-Sample Analysis without Mappability and GC Content}\label{two_sample_no_MGC}
...
`\texttt{byChr}', `\texttt{fragLen}', `\texttt{binSize}', and `\texttt{capping}'. 
...
\begin{table}
\caption{ \label{tab:mosaicsRunAll} \textbf{Summary of the arguments of `\texttt{mosaicsRunAll}' method.} In the tables (a) and (b), each cell in the second to fourth columns of the second to fourth rows corresponds to each argument. In table (c), the first column corresponds to arguments.}
\label{t:case}
\begin{center}
\begin{tabular}{llll}
\multicolumn{4}{c}{(a) Input and output files} \\
\hline
Category & Directory & File name & File format\\
\hline
ChIP sample & \texttt{chipDir} & \texttt{chipFileName} & \texttt{chipFileFormat}\\
Matched control sample & \texttt{controlDir} & \texttt{controlFileName} & \texttt{controlFileFormat}\\
Bin-level files & \texttt{binfileDir} & & \\
Peak list & \texttt{peakDir} & \texttt{peakFileName} & \texttt{peakFileFormat}\\
\hline
\multicolumn{4}{c}{} \\
\multicolumn{4}{c}{(b) Reports} \\
\hline
Category & Generate report? & Directory & File name\\
\hline
Analysis summary & \texttt{reportSummary} * & \texttt{summaryDir} & \texttt{summaryFileName}\\
Exploratory plots & \texttt{reportExploratory} * & \texttt{exploratoryDir} & \texttt{exploratoryFileName}\\
GOF plots & \texttt{reportGOF} * & \texttt{gofDir} & \texttt{gofFileName}\\
\hline
\multicolumn{4}{l}{* Reports will be generated only when these arguments are \texttt{TRUE}. Default is \texttt{FALSE}.} \\
\multicolumn{4}{c}{} \\
\multicolumn{4}{c}{(c) Tuning parameters} \\
\hline
Argument & \multicolumn{3}{l}{Explanation} \\
\hline
\texttt{byChr} & \multicolumn{3}{l}{Genome-wide analysis (\texttt{FALSE}) or chromosome-wise analysis (\texttt{TRUE})?} \\
\texttt{fragLen} & \multicolumn{3}{l}{Average fragment length.} \\
\texttt{binSize} & \multicolumn{3}{l}{Bin size.} \\
\texttt{capping} & \multicolumn{3}{l}{Cap read counts in aligned read files?} \\
\texttt{bgEst} & \multicolumn{3}{l}{Background estimation approach.} \\
\texttt{signalModel} & \multicolumn{3}{l}{Signal model.} \\
\texttt{FDR} & \multicolumn{3}{l}{False discovery rate (FDR).} \\
\texttt{maxgap} & \multicolumn{3}{l}{Distance between initial peaks for merging.} \\
\texttt{minsize} & \multicolumn{3}{l}{Minimum width to be called as a peak.} \\
\texttt{thres} & \multicolumn{3}{l}{Minimum ChIP tag counts to be called as a peak.} \\
\texttt{nCore} & \multicolumn{3}{l}{Number of CPUs used for parallel processing/computing. **} \\
\hline
\multicolumn{4}{l}{** Relevant only when \texttt{multicore} package is installed.} \\
\end{tabular}
\end{center}
\end{table}
